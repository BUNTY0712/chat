// useEffect(() => {
//   if (allUser?.data) {
//     const item = allUser.data.find(
//       (item) => item.s_seen === "1" && item.r_seen === "0"
//     );
//     if (item) {
//       play();
//     }
...
<Box
  mt={1}
  style={{
    background: "hsl(225deg 7.79% 30.2%)",
    padding: "15px",
    borderRadius: "8px",
  }}
  ref={containerRef}
...
>
  <InfiniteScroll
    ...
    endMessage={
      <p style={{ textAlign: "center" }}>{/* <b>No New Message</b> */}</p>
    }
      ...
      user.slice(-scrollVal).map((item, index) => {
        ...
        return (
          ...
          >
            ...
            {item.id === selectedTripID && <Box display="flex"></Box>}
          </Box>
        );
      })}
  </InfiniteScroll>
